...
internal static class BrzyckiCalculator
{
  ...
  public static double EstimateOneRepMax(double weightKg, int reps)
  {
    ...
    if (reps <= 0 || reps >= 37)
    {
      throw new ArgumentException("Reps must be between 1 and 36 for Brzycki formula", nameof(reps));
    }
  ...
  {
    if (targetPercentOfOneRm <= 0 || targetPercentOfOneRm > 1)
    {
      throw new ArgumentException("Target percent must be (0,1]", nameof(targetPercentOfOneRm));
    }
